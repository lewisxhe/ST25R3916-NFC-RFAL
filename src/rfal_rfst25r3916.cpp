...
#if defined(RFAL_CE_DEBUG) || defined(RFAL_ST25R3916_LISTEN_DEBUG)
#define RFAL_ST25R3916_LM_LOG(msg) do { Serial.print("[ST25R3916 LM] "); Serial.println(msg); } while (0)
#define RFAL_ST25R3916_LM_LOG_VAL(msg, val) do { Serial.print("[ST25R3916 LM] "); Serial.print(msg); Serial.println((uint32_t)(val)); } while (0)
#define RFAL_ST25R3916_LM_LOG_HEX(msg, val) do { Serial.print("[ST25R3916 LM] "); Serial.print(msg); Serial.println((uint32_t)(val), HEX); } while (0)
#else
#define RFAL_ST25R3916_LM_LOG(msg) do { } while (0)
#define RFAL_ST25R3916_LM_LOG_VAL(msg, val) do { } while (0)
#define RFAL_ST25R3916_LM_LOG_HEX(msg, val) do { } while (0)
#endif

#ifndef RFAL_ISO14443A_CRC_INTVAL
#define RFAL_ISO14443A_CRC_INTVAL 0x6363U
#endif

#ifndef ST25R3916_TOUT_OSC_STABLE
...
RfalRfST25R3916Class::RfalRfST25R3916Class(TwoWire *i2c, int int_pin) : dev_i2c(i2c), dev_spi(NULL), cs_pin(-1), int_pin(int_pin), spi_speed(0)
{
  memset(&gRFAL, 0, sizeof(rfal));
  memset(&gRfalAnalogConfigMgmt, 0, sizeof(rfalAnalogConfigMgmt));
  memset(&iso15693PhyConfig, 0, sizeof(iso15693PhyConfig_t));
...
ReturnCode RfalRfST25R3916Class::rfalListenStart(uint32_t lmMask, const rfalLmConfPA *confA, const rfalLmConfPB *confB, const rfalLmConfPF *confF, uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rxLen)
{
  ...
  RFAL_ST25R3916_LM_LOG_HEX("ListenStart mask=0x", lmMask);

  if (gRFAL.state < RFAL_STATE_INIT) {
    return ST_ERR_WRONG_STATE;
  }

  if ((rxBuf == NULL) || (rxLen == NULL) || (rxBufLen == 0U)) {
    return ST_ERR_PARAM;
  }
  ...
  if ((lmMask & RFAL_LM_MASK_NFCA) != 0U) {
    ...
    *(pPTMem++) = (confA->SEL_RES & ~RFAL_LM_NFCID_INCOMPLETE);
    ...
    RFAL_ST25R3916_LM_LOG("NFC-A listener configured");
  }
  ...
  if ((lmMask & RFAL_LM_MASK_NFCF) != 0U) {
    ...
    pPTMem = &pPTMem[RFAL_LM_SENSF_SC_LEN];

    ST_MEMCPY(pPTMem, confF->SENSF_RES, RFAL_LM_SENSF_RES_LEN);
    pPTMem[RFAL_LM_SENSF_RD0_POS] = 0x00;
    pPTMem[RFAL_LM_SENSF_RD1_POS] = 0x00;
    ...
    RFAL_ST25R3916_LM_LOG("NFC-F listener configured");
  }
  ...
  *gRFAL.Lm.rxLen   = 0;
  ...
  st25r3916ChangeRegisterBits(ST25R3916_REG_PASSIVE_TARGET,
                              (ST25R3916_REG_PASSIVE_TARGET_d_106_ac_a | ST25R3916_REG_PASSIVE_TARGET_d_212_424_1r | ST25R3916_REG_PASSIVE_TARGET_d_ac_ap2p),
                              autoResp);
  ...
  ReturnCode ret = rfalListenSetState(RFAL_LM_STATE_POWER_OFF);
  RFAL_ST25R3916_LM_LOG_VAL("ListenStart ret=", ret);
  return ret;
}
...
ReturnCode RfalRfST25R3916Class::rfalRunListenModeWorker(void)
{
  ...
  switch (gRFAL.Lm.state) {
    case RFAL_LM_STATE_POWER_OFF:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_EON) != 0U) {
        RFAL_ST25R3916_LM_LOG("External field on");
    ...
    case RFAL_LM_STATE_IDLE:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_NFCT) != 0U) {
        st25r3916ReadRegister(ST25R3916_REG_NFCIP1_BIT_RATE, &tmp);
        tmp >>= ST25R3916_REG_NFCIP1_BIT_RATE_nfc_rate_shift;
        if (tmp > ST25R3916_REG_BIT_RATE_rxrate_424) {
          tmp = ST25R3916_REG_BIT_RATE_rxrate_424;
        }
        gRFAL.Lm.brDetected = (rfalBitRate)tmp;
      }
      ...
      if (((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) && (!gRFAL.Lm.dataFlag)) {
        ...
        *gRFAL.Lm.rxLen   = (uint16_t)rfalConvBytesToBits(*gRFAL.Lm.rxLen);
        ...
        RFAL_ST25R3916_LM_LOG_VAL("Data received bits=", *gRFAL.Lm.rxLen);
        rfalCheckDisableObsMode();
    ...
    case RFAL_LM_STATE_READY_F:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) {
        ...
        *gRFAL.Lm.rxLen   = (uint16_t)rfalConvBytesToBits(*gRFAL.Lm.rxLen);
        gRFAL.Lm.dataFlag = true;
        RFAL_ST25R3916_LM_LOG_VAL("NFC-F data bits=", *gRFAL.Lm.rxLen);
    ...
    case RFAL_LM_STATE_ACTIVE_Ax:
      ...
      if ((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) {
        ...
        *gRFAL.Lm.rxLen   = (uint16_t)rfalConvBytesToBits(*gRFAL.Lm.rxLen);
        gRFAL.Lm.dataFlag = true;
        RFAL_ST25R3916_LM_LOG_VAL("NFC-A data bits=", *gRFAL.Lm.rxLen);
      } else {
        /* No listen event to process. */
      }
      break;
...
{
  RFAL_ST25R3916_LM_LOG_VAL("SleepStart state=", sleepSt);
...
{
  RFAL_ST25R3916_LM_LOG("ListenStop");
...
ReturnCode RfalRfST25R3916Class::rfalListenSetState(rfalLmState newSt)
{
  ...
  RFAL_ST25R3916_LM_LOG_VAL("SetState=", newSt);
      ...
      case RFAL_LM_STATE_IDLE:
        ...
        if ((gRFAL.Lm.mdMask & RFAL_LM_MASK_ACTIVE_P2P) != 0U) {
          ...
          if (!st25r3916IsExtFieldOn()) {
            reSetState = true;
            newState   = RFAL_LM_STATE_POWER_OFF;
          }
        }
