...
#if defined(RFAL_CE_DEBUG) || defined(RFAL_NFC_CE_DEBUG)
#define RFAL_NFC_CE_LOG(msg) do { Serial.print("[RFAL NFC CE] "); Serial.println(msg); } while (0)
#define RFAL_NFC_CE_LOG_VAL(msg, val) do { Serial.print("[RFAL NFC CE] "); Serial.print(msg); Serial.println((uint32_t)(val)); } while (0)
#define RFAL_NFC_CE_LOG_HEX(msg, val) do { Serial.print("[RFAL NFC CE] "); Serial.print(msg); Serial.println((uint32_t)(val), HEX); } while (0)
#else
#define RFAL_NFC_CE_LOG(msg) do { } while (0)
#define RFAL_NFC_CE_LOG_VAL(msg, val) do { } while (0)
#define RFAL_NFC_CE_LOG_HEX(msg, val) do { } while (0)
#endif
...
void RfalNfcClass::rfalNfcWorker(void)
{
  ...
  switch (gNfcDev.state) {
    ...
    case RFAL_NFC_STATE_LISTEN_TECHDETECT:
      ...
      err = rfalRfDev->rfalListenStart(gNfcDev.lmMask, &gNfcDev.disc.lmConfigPA, NULL, &gNfcDev.disc.lmConfigPF,
                                       (uint8_t *)gNfcDev.rxBuf.rfBuf, (uint16_t)rfalConvBytesToBits(sizeof(gNfcDev.rxBuf.rfBuf)), &gNfcDev.rxLen);
      if (err == ST_ERR_NONE) {
        RFAL_NFC_CE_LOG("Listen started");
        gNfcDev.state = RFAL_NFC_STATE_LISTEN_COLAVOIDANCE;
        rfalNfcNfcNotify(gNfcDev.state);
      } else {
        RFAL_NFC_CE_LOG_VAL("ListenStart failed err=", err);
    ...
    case RFAL_NFC_STATE_LISTEN_COLAVOIDANCE:
      ...
      if (rfalRfDev->rfalListenGetState(NULL, NULL) >= RFAL_LM_STATE_IDLE) {
        RFAL_NFC_CE_LOG("Listen field detected");
        gNfcDev.state = RFAL_NFC_STATE_LISTEN_ACTIVATION;
        rfalNfcNfcNotify(gNfcDev.state);
      }
    ...
    case RFAL_NFC_STATE_LISTEN_SLEEP:
      ...
      if (err == ST_ERR_NONE) {
        RFAL_NFC_CE_LOG("Listen activated");
        gNfcDev.devCnt    = 1U;
        gNfcDev.activeDev = gNfcDev.devList;
        gNfcDev.state     = RFAL_NFC_STATE_ACTIVATED;
        rfalNfcNfcNotify(gNfcDev.state);
        break;
      }

      if ((err == ST_ERR_BUSY) && !timerIsExpired(gNfcDev.discTmr)) {
        break;
      }

      RFAL_NFC_CE_LOG_VAL("Listen activation ended err=", err);
      rfalRfDev->rfalListenStop();
      gNfcDev.state = RFAL_NFC_STATE_START_DISCOVERY;
      rfalNfcNfcNotify(gNfcDev.state);
      break;
...
ReturnCode RfalNfcClass::rfalNfcDataExchangeGetStatus(void)
{
  ...
  if (gNfcDev.dataExErr == ST_ERR_BUSY) {
    ...
    if (gNfcDev.dataExErr == ST_ERR_SLEEP_REQ) {
      ReturnCode sleepErr;

      RFAL_NFC_CE_LOG("Entering listen sleep after data exchange");
      sleepErr = rfalRfDev->rfalListenSleepStart(RFAL_LM_STATE_SLEEP_A, gNfcDev.rxBuf.rfBuf, sizeof(gNfcDev.rxBuf.rfBuf), &gNfcDev.rxLen);
      if (sleepErr != ST_ERR_NONE) {
        gNfcDev.dataExErr = sleepErr;
      } else {
        gNfcDev.dataExErr = ST_ERR_SLEEP_REQ;
      }
    }
  }
...
ReturnCode RfalNfcClass::rfalNfcListenActivation(void)
{
  ...
  switch (lmSt) {
    ...
    case RFAL_LM_STATE_ACTIVE_Ax:
      ...
      if (isDataRcvd) {
        uint16_t rxBytes = rfalConvBitsToBytes(gNfcDev.rxLen);

        if (rfalNfcaListenerIsSleepReq(gNfcDev.rxBuf.rfBuf, rxBytes)) {
          RFAL_NFC_CE_LOG("NFC-A sleep request");
        ...
        } else if (rfalIsoDepIsRats(gNfcDev.rxBuf.rfBuf, (uint8_t)rxBytes)) {
          ...
          rfalRfDev->rfalListenSetState(RFAL_LM_STATE_CARDEMU_4A);
          rfalIsoDepInitialize();
          RFAL_NFC_CE_LOG("NFC-A RATS, starting ISO-DEP listen activation");
          EXIT_ON_ERR(ret, rfalIsoDepListenStartActivation(&atsParam, NULL, gNfcDev.rxBuf.rfBuf, gNfcDev.rxLen, rxParam));
        } else {
          RFAL_NFC_CE_LOG_VAL("Unexpected NFC-A first frame bytes=", rxBytes);
          return ST_ERR_PROTO;
        }
      }
      return ST_ERR_BUSY;
    ...
    case RFAL_LM_STATE_CARDEMU_4A:
      ...
      if (ret == ST_ERR_NONE) {
        ...
        gNfcDev.devList->nfcidLen    = 0U;
        RFAL_NFC_CE_LOG("NFC-A ISO-DEP card emulation active");
      }
      return ((ret == ST_ERR_LINK_LOSS) ? ST_ERR_PROTO : ret);

    case RFAL_LM_STATE_READY_F:

      if (isDataRcvd) {
        RFAL_NFC_CE_LOG("NFC-F first frame received");
        rfalRfDev->rfalListenSetState(RFAL_LM_STATE_CARDEMU_3);
      }
      return ST_ERR_BUSY;
    ...
    case RFAL_LM_STATE_CARDEMU_3:
      ...
      gNfcDev.devList->nfcid       = NULL;
      gNfcDev.devList->nfcidLen    = 0U;
      RFAL_NFC_CE_LOG("NFC-F Type 3 card emulation active");
      return ST_ERR_NONE;
