...
#if defined(RFAL_CE_DEBUG) || defined(RFAL_ISODEP_CE_DEBUG)
#define RFAL_ISODEP_CE_LOG(msg) do { Serial.print("[ISO-DEP CE] "); Serial.println(msg); } while (0)
#define RFAL_ISODEP_CE_LOG_VAL(msg, val) do { Serial.print("[ISO-DEP CE] "); Serial.print(msg); Serial.println((uint32_t)(val)); } while (0)
#define RFAL_ISODEP_CE_LOG_HEX(msg, val) do { Serial.print("[ISO-DEP CE] "); Serial.print(msg); Serial.println((uint32_t)(val), HEX); } while (0)
#else
#define RFAL_ISODEP_CE_LOG(msg) do { } while (0)
#define RFAL_ISODEP_CE_LOG_VAL(msg, val) do { } while (0)
#define RFAL_ISODEP_CE_LOG_HEX(msg, val) do { } while (0)
#endif
...
/*******************************************************************************/
ReturnCode RfalNfcClass::isoDepReSendControlMsg(void)
{
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepListenStartActivation(rfalIsoDepAtsParam *atsParam, const rfalIsoDepAttribResParam *attribResParam, uint8_t *buf, uint16_t bufLen, rfalIsoDepListenActvParam actParam)
{
  uint8_t *txBuf;
  uint8_t  bufIt;
  uint8_t *buffer;
  ...
  if ((buf == NULL) || (actParam.rxBuf == NULL) || (actParam.rxLen == NULL) || (actParam.isRxChaining == NULL)) {
    return ST_ERR_PARAM;
  }

  bufIt        = 0U;
  txBuf        = (uint8_t *)actParam.rxBuf;
  buffer       = buf;
  ...
  if (*buffer == RFAL_ISODEP_CMD_RATS) {
    ...
    if (actParam.isoDepDev != NULL) {
      ST_MEMCPY((uint8_t *)&actParam.isoDepDev->activation.A.Poller.RATS, buffer, sizeof(rfalIsoDepRats));
    }
    ...
    txBuf[bufIt++] = (RFAL_ISODEP_ATS_HIST_OFFSET + atsParam->hbLen);
    txBuf[bufIt++] = (RFAL_ISODEP_ATS_T0_TA_PRESENCE_MASK | RFAL_ISODEP_ATS_T0_TB_PRESENCE_MASK | RFAL_ISODEP_ATS_T0_TC_PRESENCE_MASK | atsParam->fsci);
    txBuf[bufIt++] = atsParam->ta;
    txBuf[bufIt++] = ((atsParam->fwi << RFAL_ISODEP_ATS_TB_FWI_SHIFT) | (atsParam->sfgi & RFAL_ISODEP_ATS_TB_SFGI_MASK));
    txBuf[bufIt++] = (uint8_t)((atsParam->didSupport) ? RFAL_ISODEP_ATS_TC_DID : 0U);

    if (atsParam->hbLen > 0U) {
      ST_MEMCPY(&txBuf[bufIt], atsParam->hb, atsParam->hbLen);
      bufIt += atsParam->hbLen;
    }

    gIsoDep.state = ISODEP_ST_PICC_ACT_ATS;
    RFAL_ISODEP_CE_LOG_VAL("RATS received, ATS len=", bufIt);
    ...
    RFAL_ISODEP_CE_LOG("ATTRIB listen activation is not implemented");
  ...
  if (actParam.isoDepDev != NULL) {
    ...
    actParam.isoDepDev->info.dFWT = 0U;
...
ReturnCode RfalNfcClass::rfalIsoDepListenGetActivationStatus(void)
{
  ...
  switch (err) {
    ...
    case ST_ERR_LINK_LOSS:
      RFAL_ISODEP_CE_LOG("Activation link loss after ATS");
      return err;
  ...
  txBuf = (uint8_t *)gIsoDep.actvParam.rxBuf;
  dri   = RFAL_BR_KEEP;
  dsi   = RFAL_BR_KEEP;
  bufIt = 0U;
  ...
  if (gIsoDep.state == ISODEP_ST_PICC_ACT_ATS) {
    if ((((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_STARTBYTE_POS] & RFAL_ISODEP_PPS_MASK) == RFAL_ISODEP_PPS_SB) {
      RFAL_ISODEP_CE_LOG_HEX("PPS PPSS=0x", ((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_STARTBYTE_POS]);
      RFAL_ISODEP_CE_LOG_HEX("PPS PPS0=0x", ((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_PPS0_POS]);
      if (((((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_STARTBYTE_POS] & RFAL_ISODEP_DID_MASK) != gIsoDep.did) ||
        ...
        RFAL_ISODEP_CE_LOG("Ignoring invalid PPS");
        isoDepReEnableRx((uint8_t *)gIsoDep.actvParam.rxBuf, sizeof(rfalIsoDepBufFormat), gIsoDep.actvParam.rxLen);
        return ST_ERR_BUSY;
      }

      if (((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_PPS0_POS] == RFAL_ISODEP_PPS0_PPS1_PRESENT) {
        uint8_t newdri = ((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_PPS1_POS] & RFAL_ISODEP_PPS1_DxI_MASK;
        uint8_t newdsi = (((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_PPS1_POS] >> RFAL_ISODEP_PPS1_DSI_SHIFT) & RFAL_ISODEP_PPS1_DxI_MASK;

        RFAL_ISODEP_CE_LOG_HEX("PPS PPS1=0x", ((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_PPS1_POS]);
        RFAL_ISODEP_CE_LOG_VAL("PPS DSI=", newdsi);
        RFAL_ISODEP_CE_LOG_VAL("PPS DRI=", newdri);
        ...
        if (((!RFAL_SUPPORT_BR_CE_A_106) && ((dsi == RFAL_BR_106) || (dri == RFAL_BR_106))) ||
          ...
          RFAL_ISODEP_CE_LOG("Unsupported PPS bitrate");
          return ST_ERR_PROTO;
        }
      }
      ...
      RFAL_ISODEP_CE_LOG("PPS handled");
    }
  }
  ...
  gIsoDep.state = ISODEP_ST_PICC_RX;
  RFAL_ISODEP_CE_LOG("Activation complete");
  return ST_ERR_NONE;
}

/*******************************************************************************/
ReturnCode RfalNfcClass::isoDepDataExchangePICC(void)
{
  uint8_t    rxPCB;
  ReturnCode ret;
  ...
  switch (gIsoDep.state) {
    ...
    case ISODEP_ST_PICC_SWTX:
      if (!isoDepTimerisExpired(gIsoDep.WTXTimer)) {
        return ST_ERR_BUSY;
      }
    ...
    case ISODEP_ST_PICC_SDSL:
      if (!rfalRfDev->rfalIsTransceiveInTx()) {
        rfalIsoDepInitialize();
        RFAL_ISODEP_CE_LOG("Deselect complete");
  ...
  if (isoDep_PCBisSBlock(rxPCB)) {
    ...
    if (isoDep_PCBisSDeselect(rxPCB) && ((*gIsoDep.rxLen - gIsoDep.hdrLen) == ISODEP_SDSL_INF_LEN)) {
      ...
      RFAL_ISODEP_CE_LOG("Deselect requested");
      return ST_ERR_BUSY;
    }
    ...
    } else if (isoDep_PCBisRNAK(rxPCB)) {
      if (isoDep_GetBN(rxPCB) == gIsoDep.blockNumber) {
        if (!isoDep_PCBisIBlock(gIsoDep.lastPCB)) {
          isoDepReSendControlMsg();
        } else {
    ...
    *gIsoDep.rxLen -= gIsoDep.hdrLen;
    ...
    isoDepTimerStart(gIsoDep.WTXTimer, isoDep_WTXAdjust(rfalConv1fcToMs(gIsoDep.fwt)));
    gIsoDep.state = ISODEP_ST_PICC_SWTX;
    RFAL_ISODEP_CE_LOG_VAL("I-Block received, INF len=", *gIsoDep.rxLen);
    return ST_ERR_NONE;
  } else {
    /* Unexpected block; listener remains in RX. */
  }
